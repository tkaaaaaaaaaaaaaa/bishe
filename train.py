...
3、训练好的权值文件保存在logs文件夹中，每个训练世代（Epoch）包含若干训练步长（Step），每个训练步长（Step）进行一次梯度下降。
   如果只是训练了几个Step是不会保存的，Epoch和cStep的概念要捋清楚一下。
'''
if __name__ == "__main__":
    ...
    if distributed:  # 分布式训练
        dist.init_process_group(backend="nccl")
        # 使用pytorch进行分布式训练，需要指定 local_rank，主机 local_rank = 0
        local_rank  = int(os.environ["LOCAL_RANK"])   # os.environ环境变量，字典类型。
    ...
    if model_path != '':
        ...
        #------------------------------------------------------#
        model_dict      = model.state_dict()  # 一个字典对象，记录了模型每层的参数
    ...
    #----------------------#
    if local_rank == 0:
        time_str        = datetime.datetime.strftime(datetime.datetime.now(),'%Y_%m_%d_%H_%M_%S')  # 时间日期
        log_dir         = os.path.join(save_dir, "loss_" + str(time_str))  # 地址信息
        loss_history    = LossHistory(log_dir, model, input_shape=input_shape)  # 最终loss信息
    else:
        loss_history    = None
